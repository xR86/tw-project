...
create or replace package body FreeCodeCamp is
  ...
  ---------------------------------------------------------
    ...
    function F_FILTER (v_hassolution IN CHAR,v_task_name VARCHAR2, v_completeddate DATE) 
    return VARCHAR2 is
      v_stat VARCHAR2(500);
    begin
      CASE
        WHEN v_task_name IS NOT NULL THEN
          --CASE 1     
          CASE
            WHEN v_hassolution IS NOT NULL THEN
              --CASE 1.1         
              CASE
                WHEN v_completeddate IS NOT NULL THEN
                  --CASE 1.1.1          
                  v_stat := 'SELECT * FROM SOLVEDTASKSVIEW WHERE TASK_NAME=v_task_name '|| 
                                                 'AND HASSOLUTION=:hassolution '|| 
                                                 'AND TO_DATE(TRUNC(COMPLETEDDATE),''DD/MM/YY'')=TO_DATE(:completeddate,''DD/MM/YY'')';
                ELSE
                  --CASE 1.1.2  
                  v_stat := 'SELECT * FROM SOLVEDTASKSVIEW WHERE TASK_NAME=:task_name '|| 
                                                 'AND HASSOLUTION=:hassolution';
              END CASE; --END 1.1.*
            ELSE
              --CASE 1.2
          ...
          CASE
            WHEN v_hassolution IS NOT NULL THEN
              ...
              CASE
                WHEN v_completeddate IS NOT NULL THEN
                  --CASE 2.1.1          
                  v_stat := 'SELECT * FROM SOLVEDTASKSVIEW WHERE HASSOLUTION=:hassolution '|| 
                                                 'AND TO_DATE(TRUNC(COMPLETEDDATE),''DD/MM/YY'')=TO_DATE(:completeddate,''DD/MM/YY'')';
                ELSE
                  --CASE 2.1.2  
                  v_stat := 'SELECT * FROM SOLVEDTASKSVIEW WHEREHASSOLUTION=:hassolution';
              END CASE; --END 2.1.*
            ELSE
              --CASE 2.2
              CASE
                WHEN v_completeddate IS NOT NULL THEN
                  --CASE 2.2.1
                  v_stat := 'SELECT * FROM SOLVEDTASKSVIEW WHERE TO_DATE(TRUNC(COMPLETEDDATE),''DD/MM/YY'')=TO_DATE(:completeddate,''DD/MM/YY'')';
                ELSE
                  --CASE 2.2.2
                  v_stat := 'SELECT * FROM SOLVEDTASKSVIEW';
              END CASE; --END 2.2.*
          END CASE; --END 2.*
      END CASE;
      return v_stat;
    ...
    --select f_filter(NULL,'ceva',TO_DATE('12/06/15','DD/MM/YY')) from dual
